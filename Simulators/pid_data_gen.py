...
log_file = open('training_data.csv', mode='w', newline='')
log_writer = csv.writer(log_file)

# Write the header row
log_writer.writerow(['theta', 'omega', 'velocity', 'targetvelocity', 'acc', 'x_position'])
...
while not done:
    ...
    if -5 * np.pi <= theta <= 5 * np.pi:
        ...
        # Calculate target angle using PID controllers
        target_theta = -speed_pid.output(
            geom.v2ang(h, g, targetvelocity),
            -geom.v2ang(h, g, velocity),
            dt
        )
        acc = -angle_pid.output(target_theta, theta + noise[0], dt)

        # Log the data
        x_position = distance  # Assuming 'distance' represents the x-axis position
        log_writer.writerow([theta, omega, velocity, targetvelocity, acc, x_position])
    # -------------------------------------------------------------------
    #                          Draw Stuff
    # -------------------------------------------------------------------
